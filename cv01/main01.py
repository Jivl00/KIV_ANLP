...
def main(config: dict):
    ...
    model = DenseNet(dp=DP).to(device) if config["model"] == "dense" else Net(dp=DP).to(device)
    wandb.watch(model, log="all")
    optimizer = optim.SGD(model.parameters(), lr=LR) if config["optimizer"] == "sgd" else optim.Adam(model.parameters(), lr=LR)
